package com.prateek.ui.login;

import org.springframework.beans.factory.annotation.Autowired;

import com.prateek.service.security.RegisterUserService;
...
public class SignupFormFactory {
	...
	private class SignupForm{
		...
		public SignupForm init() {
			...
			passwordAgainField = new PasswordField("Confirm Password");
			
			saveButton.addClickListener(new Button.ClickListener() {				
				@Override
				public void buttonClick(ClickEvent event) {					
					if(!passwordAgainField.getValue().equals(passwordField.getValue())) {
						new Notification("Error","Password do not match!", Type.ERROR_MESSAGE);
						return;
					}					
